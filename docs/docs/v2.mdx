...
Solito 2 introduces [`SolitoImage`](/usage/image), which wraps `next/image` on Web and [`react-native-fast-image`](https://github.com/DylanVann/react-native-fast-image) on iOS & Android.
