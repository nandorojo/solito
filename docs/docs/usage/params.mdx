...
`useParam` is a hook that lets you read screen parameters on from both Next.js and React Native. On Native, it reads React Navigation params, and on Web, it reads query params from `next/router`.
...
You'll `useParam` this instead of `useRoute().params` from React Navigation, and instead of `useRouter().query` from Next.js.
