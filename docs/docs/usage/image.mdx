...
It's useful to get acquainted with Next.js' Image component before using this one.

Please reference the Next.js [`Image` component](https://nextjs.org/docs/api-reference/next/image) for props usage.
...
You can style the image using the `style` prop. It accepts a React Native object.
...
For example, say you use Cloudinary as your image CDN. Your loader would look like this:
...
If you still want to use Expo Go, you can import from `solito/image/expo` instead. This uses React Native's vanilla `Image` component under the hood.

For a production app, I recommend using `solito/image` with `react-native-fast-image` and a custom dev client, since React Native's vanilla image is known to have bugs.
